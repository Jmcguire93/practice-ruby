...
### First Store, multiple arguements in initialize method ###
...
### Second store, takes in options hash in initialize method ###
...
#### Third store, uses single options array instead of hash ####

class Store
  attr_reader :product, :color, :price
  
  def initialize(input_options)
    @product = input_options[0]
    @color = input_options[1]
    @price = input_options[2]
  end
